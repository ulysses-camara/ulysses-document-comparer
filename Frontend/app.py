    ...
    if (request.method == "GET"):
        ...
        if (resp.status_code==200):
            ...
            if (st_mode == '0'):
                ...
                scores_final = [results[i]["score_final"] for i in range(10)]
                return render_template("index.html", docs=top_docs, names=labels, query=query, 
                                    ids=ids, len=len(labels), scores=scores, scores_normalized=scores_normalized, scores_final=scores_final, st_search=False)
            else:
                ...
                scores_final = [results[i]["score_final"] for i in range(10)]
                return render_template("index.html", docs=top_docs, names=labels, query=query, 
                                    len=len(labels), scores=scores, scores_final=scores_final, st_search=True)
...
def submit(query):
    ...
    scores_final = [r[3] for r in form_results]
    if (st_search == '0'):
        results = [{"id": docs[i], "class": classes[i], "score_bm25": scores[i], "score_normalized": scores_normalized[i], "score_final": scores_final[i], "tipo": "PR"} for i in range(len(form_results))]
    else:
        results = [{"id": docs[i], "class": classes[i], "score_bm25": scores[i], "score_normalized": scores_normalized[i], "score_final": scores_final[i], "tipo": "ST"} for i in range(len(form_results))]
